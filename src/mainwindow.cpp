    ...
    QMainWindow(parent)
{
    ...
    //test->resize(20, 10);
    new QSvgWidget("src/image/embeddedts-color-outline.svg", test);
    QSvgWidget *logo = new QSvgWidget("src/image/embeddedts-color-outline.svg", test);
    logo->resize(600, 140);
    //test->resize(40, 20);
    vbox->setAlignment(logo, Qt::AlignHCenter);
    vbox->addWidget(test);
    //vbox->addWidget(test);
 //   vbox->setAlignment(test, Qt::AlignLeft);
#endif
